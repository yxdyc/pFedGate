...
class Aggregator(ABC):
    ...
    global_learners_ensemble: List[Learner],  to store the global models
    ...
    def __init__(
        ...
        self.total_clients_train_num = self.clients_weights.sum()

        self.clients_weights = self.clients_weights / self.total_clients_train_num
        ...
        self.c_round = 0
        # Cross evaluation to examine the personalization effect
        self.cross_eval_res_val = np.full((len(self.clients), len(self.clients)), fill_value=-1, dtype=np.float64)
        self.cross_eval_res_test = np.full((len(self.clients), len(self.clients)), fill_value=-1, dtype=np.float64)
        self.client_level_mask = [None for _ in range(len(self.clients))]
        self.cross_eval_rounds = [1, 285, 300]  # used for femnist01, study the personalization effect
        self.client_mask_save_rounds = [155, 185]  # used for femnist01, study the personalization effect
        self.cross_eval_rounds = []
        self.client_mask_save_rounds = []
    ...
    def write_logs(self):
        ...
        for global_logger, clients in [
            ...
            for client_id, client in enumerate(clients):
                ...
                test_acc_all_clients.append(test_acc)
            ...
            try:
                ...
                wandb.log({f"node_agg/train/loss_meta_model": global_train_loss,
                           f"node_agg/train/metric": global_train_acc,
                           ...
                           f"node_agg/test/client_counter": self.c_round}, commit=True)
...
class PersonalizedAggregator(CentralizedAggregator):
    r"""
    Clients do not synchronize their models, instead they only synchronize optimizers, when needed.
